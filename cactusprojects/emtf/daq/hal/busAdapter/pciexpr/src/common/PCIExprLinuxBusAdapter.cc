...
void HAL::PCIExprLinuxBusAdapter::write64( PCIDeviceIdentifier& device,
  ...
  try {
/// KK what exactly does this function do?
    mwrite(device_d, &data, 8, address);
  }
  catch ( xpci::exception::IOError &e ) { 	
    std::stringstream text;
    text << "error during write to address  " 
...
/// KK what exactly does this function do?
    mread(device_d, result, 8, address);
  } catch( xpci::exception::IOError &e ) {
    std::stringstream text;
    text << "error during read from address  " 
         << std::hex << address 
...
void HAL::PCIExprLinuxBusAdapter::writeBlock64( PCIDeviceIdentifier& device,
  ...
  const int nWords = length/8;
  uint64_t write_buf[nWords];
...
void HAL::PCIExprLinuxBusAdapter::readBlock64(  PCIDeviceIdentifier& device,
					      uint64_t startAddress,
					      uint64_t length,
					      char *buffer,
					      HalAddressIncrement addressBehaviour) 
