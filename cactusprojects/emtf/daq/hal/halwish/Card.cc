...
// create new card (factory method)
  int Card::create(ClientData clientData, Tcl_Interp * interp,
              int argc, char* argv[]){
    if(argc!=4){
    ...
    try {
            Card* newCard = new Card(baseAddress,addressMap);
            newCard->name=name;
            Tcl_CreateCommand(interp,name,Card::command,(ClientData) newCard,
                              (Tcl_CmdDeleteProc *) Card::deleteCard);
            return TCL_OK;
...
// delete card
  void Card::deleteCard(ClientData clientData){
    delete (Card*) clientData;
  }

// execute command on card
int Card::command(ClientData clientData, Tcl_Interp * interp,
                  int argc, char* argv[]){
        int status = TCL_OK;
        if(argc==2){
                // one argument: read
                uint32_t value;
                try{
                        ((Card*)clientData)->card.read(argv[1],&value);
                ...
                if(Tcl_GetInt(interp,argv[2],&value)==TCL_OK){
                        try{
                                ((Card*)clientData)->card.write(argv[1],(uint32_t)value);
                                interp->result = "";
                                status = TCL_OK;
                        } catch ( HardwareAccessException & e ) {
                                sprintf(interp->result, "HAL error: %s", e.what());
...
// constructor (private)
  Card::Card(uint32_t baseAddress,char* addressMap):  busAdapter(), 
