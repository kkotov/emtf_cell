...
uint64_t HAL::AddressTableItem::getMask() const {
  return mask;
}
...
uint64_t HAL::AddressTableItem::countBits( uint64_t word ) const {
  uint64_t result = 0;
  for( int ic=0; ic<64; ic++ ) {
    if ( (word & 0x0000000000000001ULL) == 1 ) result++;
    word >>= 1;
  }
  return result;
}
...
uint64_t HAL::AddressTableItem::applyToMask( uint64_t data,
  ...
  if ( mask == 0 ) {
    ...
    if ( MaskBoundaryCheck ) {
      uint64_t help = mask;
      while ( (help & 0x0000000000000001ULL) == 0 ) help >>= 1;
      maskOk = ( (~ help & data) == 0 );
      if ( ! maskOk ) {
        std::stringstream text;
        text << "Data bits set outside of mask.\n"
			 ...
			 << "  Data : " << std::hex << data << "\n"  
			 << "  Mask : " << mask
			 << "      (HAL::AddressTableItem64::applyToMask)" 
			 << std::ends;
        std::string textStr = text.str();
    ...
    uint64_t help = mask;
    while ( (help & 0x0000000000000001ULL) == 0 ) {
      help >>= 1;
      data <<= 1;
    }
  }
...
uint64_t HAL::AddressTableItem::applyFromMask( uint64_t data,
						    bool MaskBoundaryCheck ) const 
  throw( HAL::MaskBoundaryException ) {
  if ( mask == 0 ) {
    data = 0;
