...
HAL::SequencerCommand* HAL::SetBitFactory::create( std::list<std::string>& arguments, 
  ...
  if ( arguments.size() < 1 || arguments.size() > 3 ) {
    throw ( HAL::SequencerSyntaxError("setBit command must have one to three arguments."));
  }
  ...
  if ( ! addressTable.exists( item ) ) {
    std::string text = "Item \"" + item +
      ...
      "\n    (HAL::SetBitFactory::create)";
    throw (HAL::NoSuchItemException( text, __FILE__, __LINE__, __FUNCTION__ ));
  }
  
  HAL::SetBitCommand* commandPtr = new HAL::SetBitCommand( item, sequence );
