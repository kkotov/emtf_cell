...
HAL::SequencerCommand* HAL::WriteFactory::create( std::list<std::string>& arguments, 
  ...
  if ( arguments.size() < 2 || arguments.size() > 4 ) {
    throw ( HAL::SequencerSyntaxError("write command must have two to four arguments."));
  }
  ...
  if ( ! addressTable.exists( item ) ) {
    std::string text = "Item \"" + item +
      ...
      "\n    (HAL::WriteFactory::create)";
    throw (HAL::NoSuchItemException( text, __FILE__, __LINE__, __FUNCTION__ ));
  }

  HAL::WriteCommand* commandPtr = new HAL::WriteCommand (item, sequence); 
