...
uint64_t HAL::AddressTable::getMask( std::string item ) const 
  throw( HAL::NoSuchItemException ) {
...
uint64_t HAL::AddressTable::applyToMask( std::string item, 
					      uint64_t data,
...
uint64_t HAL::AddressTable::applyFromMask( std::string item, 
						uint64_t data,
						bool MaskBoundaryCheck ) const 
  throw( HAL::NoSuchItemException, HAL::MaskBoundaryException ) {
