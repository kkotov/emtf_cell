...
keyTemplate = {'hardware':'EMTF_HW/v', 'algo':'EMTF_ALGO/v', 'processor':'EMTF_MTF7_base_conf/v', 'daq':'EMTF_AMC13_base_conf/v', 'key':'EMTF_Base_Key/v', 'infraKey':'EMTF_Base_Infra_Key/v'} #'rs':'EMTF_RS/v',
...
for kind,table in confTable.iteritems() :
    ...
    query = "select ID from CMS_TRG_L1_SUBSYSTEM.{0} where ID like '{1}%';".format( table, keyTemplate[kind] );
...
for kind,xmlText in payload.iteritems():
    ...
    query1 = "insert into CMS_TRG_L1_SUBSYSTEM.{0} (ID,CREATION_DATE,DESCRIPTION,CONF,AUTHOR) values ('{1}',TO_TIMESTAMP_TZ('{2}', 'DD-Mon-RR HH24:MI:SS TZHTZM'),'{3}',{4},'{5}');".format(
                 confTable[kind],
...
if 'daq' in payload or 'processor' in payload:
    middleQuery = "insert into CMS_TRG_L1_SUBSYSTEM.EMTF_INFRA_KEYS (ID,CREATION_DATE,DESCRIPTION,AUTHOR,AMC13,MTF7) values ('{0}',TO_TIMESTAMP_TZ('{1}', 'DD-Mon-RR HH24:MI:SS TZHTZM'),'{2}','{3}'".format(
    ...
    if 'daq' in payload :
        ...
        middleQuery += ",(select AMC13 from CMS_TRG_L1_SUBSYSTEM.EMTF_INFRA_KEYS where ID='{0}')".format( ''.join([ keyTemplate['infraKey'], str(keyVersion['infraKey'])]) )

    if 'processor' in payload:
        middleQuery += ",'{0}'".format( ''.join([ keyTemplate['processor'], str(keyVersion['processor'] + 1)]) )
    else :
        middleQuery += ",(select MTF7 from CMS_TRG_L1_SUBSYSTEM.EMTF_INFRA_KEYS where ID='{0}')".format( ''.join([ keyTemplate['infraKey'], str(keyVersion['infraKey'])]) )
...
#  top level
topQuery = "insert into CMS_TRG_L1_SUBSYSTEM.EMTF_KEYS (ID,CREATION_DATE,DESCRIPTION,AUTHOR,ALGO,HW,INFRA) values ('{0}',TO_TIMESTAMP_TZ('{1}', 'DD-Mon-RR HH24:MI:SS TZHTZM'),'{2}','{3}'".format(
