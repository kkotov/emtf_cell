#ifndef __PCIExprDevice
#define __PCIExprDevice
...
*            the PCI-interface.) 
*            
*            The  PCIExprDevice  implements the  interface  to access  the 
...
class PCIExprDevice : public HardwareDevice {
...
public:
   ...
   * addressTable exists as long as the PCIExprDevice (only a reference is
   * passed to the PCIExprDevice so that a single table can be shared among
   * several PCIExprDevices.) The same is valid for the busAdapter.
   *
   ...
   */
  PCIExprDevice( PCIAddressTable & addressTable,
             PCIExprLinuxBusAdapterInterface & pciBusAdapter,
             uint32_t vendorID,
             uint32_t deviceID,
             uint32_t index,
	     bool swapFlag = false);
   ...
   *        should be addressed with this PCIExprDevice.
   * @param swapFlag can be set in order to use big-endian devices
   *        with the HAL library. (Usually PCI peripherals are 
   *        little endian, but there are exotic cards like the 
   *        Myrinet NICs with the LANAI9 processor which are big endian.
  ...
  void hardwareWrite64( const GeneralHardwareAddress& pciAddress, 
                      uint64_t data,
  ...
  void hardwareRead64( const GeneralHardwareAddress& pciAddress, 
                     uint64_t* result,
                     uint64_t offset = 0 ) const
    throw (IllegalValueException,
           BusAdapterException);
 ...
 /**
  ...
  void hardwareReadBlock64( const GeneralHardwareAddress& pciAddress, 
                          uint64_t length,
                          char *buffer,
                          HalAddressIncrement addressBehaviour = HAL_DO_INCREMENT,
                          uint64_t offset = 0) const
  ...
  void hardwareWriteBlock64( const GeneralHardwareAddress& pciAddress, 
                           uint64_t length,
                           char *buffer,
                           HalAddressIncrement addressBehaviour = HAL_DO_INCREMENT,
                           uint64_t offset = 0) const
   ...
   */
  uint64_t getPCIBusAddress( uint64_t iBar )
    throw (BusAdapterException);
...
private:
  ...
  std::vector<uint64_t> baseRegisters;
};

} /* namespace HAL */

#endif /* __PCIExprDevice */
